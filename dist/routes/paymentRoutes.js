...
function paymentRoutes(db) {
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const result = await db.run(`INSERT INTO payments (user_id, amount, date)
                 VALUES (?, ?, ?)`, [payment.user_id, payment.amount, payment.date]);
    ...
    router.get('/user/:userId/balance', async (req, res) => {
        try {
            ...
            const salesResult = await db.get(`SELECT COALESCE(SUM(final_amount), 0) as total_sales
                 FROM sales
                 WHERE user_id = ?`, userId);
            // Get total payments
            const paymentsResult = await db.get(`SELECT COALESCE(SUM(amount), 0) as total_payments
                 FROM payments
                 WHERE user_id = ?`, userId);
    ...
    router.get('/user/:userId', async (req, res) => {
        try {
            const payments = await db.all(`SELECT * FROM payments
                 WHERE user_id = ?
                 ORDER BY date DESC`, req.params.userId);
            res.json(payments);
        }
        catch (error) {
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const result = await db.run(`UPDATE payments 
                 SET amount = ?, date = ?
                 WHERE id = ?`, [payment.amount, payment.date, paymentId]);
            if (result.changes === 0) {
